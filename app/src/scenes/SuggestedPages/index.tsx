	...
	return (
		<div className='mx-auto px-5 py-5' style={{ maxWidth: 800 }}>
			...
			<div className='py-3 grid gap-2 grid-flow-row sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
				{pages.map((page, index) => (<SuggestedPost key={`${page.slug}-${index}`} page={page} />))}
			</div>
		</div>
	);
};
...
function SuggestedPost({page}: {page: Page}) {
	...
	return (
		<Link href={`/${page.slug}`} passHref>
			<a className='flex gap-2 flex-col p-2 dark:bg-gray-800 bg-gray-100 rounded-md shadow-md hover:scale-[0.98] transition-transform'>
